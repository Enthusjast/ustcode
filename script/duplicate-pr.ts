...
import { createUstcode } from "@enthusjast/ustcode-sdk"
...
`)
  ...
  const ustcode = await createUstcode({ port: 0 })
  ...
  try {
    ...
    const session = await ustcode.client.session.create()
    const result = await ustcode.client.session
      .prompt({
