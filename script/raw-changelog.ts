...
const repo = process.env.GH_REPO ?? "Enthusjast/ustcode"
const bot = ["actions-user", "github-actions[bot]", "ustcode"]
const team = [...bot]
...
function section(areas: Set<string>) {
  const priority = ["core", "tui", "tauri", "sdk", "plugin", "extensions/vscode", "github"]
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/ustcode packages/sdk packages/plugin sdks/vscode`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/ustcode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/ustcode/")) areas.add("core")
