...
The ustcode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@enthusjast/ustcode-llm` tool loop after replacing its
...
- plugins get global "ustcode" instance like in that post i showed
- ustcode instance has stuff like `ustcode.session.prompt()` or
  `ustcode.tool.register({...})`
...
- decide stateless hosted-tool continuation behavior; reconstructed hosted output can replay as a stored `item_reference` when `store !== false`, while `store: false` intentionally omits the unavailable reference path
- decide whether to preserve deprecated `@enthusjast/ustcode-llm` orchestration exports
