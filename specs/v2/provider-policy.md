...
```text
action:   provider.use
resource: provider ID, such as openai-compatible or company-ai
```
...
To deny all providers except OpenAI-Compatible:
...
```text
provider.use / gateway -> allow
provider.use / openai-compatible    -> deny
```
...
{
  "experimental": {
    "policies": [{ "effect": "allow", "action": "provider.use", "resource": "openai-compatible" }],
  },
}
```
...
{
  "experimental": {
    "policies": [{ "effect": "deny", "action": "provider.use", "resource": "openai-compatible" }],
  },
}
```
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether ustcode permits an operation through its managed execution paths.
...
```jsonc
{
  "disabled_providers": ["openai-compatible", "dynamic"],
}
```
...
```jsonc
{
  "experimental": {
    "policies": [
      { "effect": "deny", "action": "provider.use", "resource": "openai-compatible" },
      { "effect": "deny", "action": "provider.use", "resource": "dynamic" },
    ],
  },
}
```
...
```jsonc
{
  "enabled_providers": ["gateway", "openai-compatible"],
}
```
...
```jsonc
{
  "experimental": {
    "policies": [
      { "effect": "deny", "action": "provider.use", "resource": "*" },
      { "effect": "allow", "action": "provider.use", "resource": "gateway" },
      { "effect": "allow", "action": "provider.use", "resource": "openai-compatible" },
    ],
  },
}
```
