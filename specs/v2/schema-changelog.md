...
This document covers meaningful contract changes introduced on the `feat/ustcode-embedded-api` branch since its divergence from `origin/main`. Mechanical file moves and internal refactors are omitted unless they changed stored data, replay behavior, public HTTP or SDK shapes, or model-facing tool contracts.
...
- Canonical tagged tool file sources in `@enthusjast/ustcode-llm`.
...
- Spill oversized model-facing tool text into globally unique files under UstCode's shared tool-output directory.
...
- Provider-executed hosted results are lowered to stored item references instead of rejecting assistant history.
- OpenAI-compatible Chat `reasoning_content` now round-trips through canonical continuation parts.
...
- Reject chronological system updates that would split a local tool call from its result across provider protocols; use wrapped user fallback when native system-update placement is unsupported.
...
- Canonicalize ambient instruction traversal boundaries, honor `USTCODE_DISABLE_PROJECT_CONFIG`, and make non-empty aggregate updates explicitly supersede previously loaded instructions.
