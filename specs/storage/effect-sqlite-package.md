...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an ustcode storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/ustcode` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No ustcode paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- `@enthusjast/ustcode-effect-drizzle-sqlite`
...
- Drizzle Effect Postgres current RC (`drizzle-team/drizzle-orm` rc-4 branch):
  - `drizzle-orm/src/effect-core/query-effect.ts`
  - `drizzle-orm/integration-tests/tests/pg/effect-sql.test.ts`
- SQLite Effect branch/reference (`drizzle-team/drizzle-orm` beta16 branch):
  - `drizzle-orm/src/up-migrations/effect-sqlite.ts`
  - `drizzle-orm/integration-tests/tests/sqlite/effect-sql.test.ts`
  - `drizzle-orm/type-tests/sqlite/effect.ts`
- Effect SQLite client source of truth (`Effect-TS/effect` sql packages):
  - `packages/sql/sqlite-bun/src/SqliteClient.ts`
  - `packages/sql/sqlite-node/test/Client.test.ts`
  - `packages/sql/sqlite-node/test/SqliteMigrator.test.ts`
...
- Ustcode-specific path/channel selection stays in `packages/ustcode`.

## Ustcode Adoption Notes

These are not package requirements, but they matter for the later ustcode adoption PR.

The current `packages/ustcode/src/storage/db.ts` has two non-obvious semantics that the ustcode wrapper must preserve when it consumes this adapter:
...
The ustcode wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving ustcode to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@enthusjast/ustcode-effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@enthusjast/ustcode-effect-drizzle-sqlite` as a dependency of `packages/ustcode`.
5. Port `packages/ustcode/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus ustcode-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of ustcode's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then ustcode can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay ustcode-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/ustcode`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not ustcode domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/ustcode` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing ustcode's current database runtime.
