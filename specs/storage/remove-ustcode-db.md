# Remove `packages/ustcode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/ustcode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the ustcode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/ustcode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/ustcode/src/account/repo.ts`
- `packages/ustcode/src/cli/cmd/db.ts`
- `packages/ustcode/src/cli/cmd/import.ts`
- `packages/ustcode/src/cli/cmd/stats.ts`
- `packages/ustcode/src/control-plane/workspace.ts`
- `packages/ustcode/src/index.ts`
- `packages/ustcode/src/node.ts`
- `packages/ustcode/src/permission/index.ts`
- `packages/ustcode/src/project/project.ts`
- `packages/ustcode/src/server/projectors.ts`
- `packages/ustcode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/ustcode/src/server/shared/fence.ts`
- `packages/ustcode/src/session/message-v2.ts`
- `packages/ustcode/src/session/projectors.ts`
- `packages/ustcode/src/session/prompt.ts`
- `packages/ustcode/src/session/session.ts`
- `packages/ustcode/src/session/todo.ts`
- `packages/ustcode/src/storage/db.ts`
- `packages/ustcode/src/sync/index.ts`
- `packages/ustcode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy ustcode database wrapper; `packages/ustcode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the ustcode wrapper.
...
Status: Completed. `SyncEvent` and the ustcode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy ustcode database wrapper.
...
- `control-plane/workspace.ts` has local Effect wrappers around `Database.use`.
...
- Then migrate `control-plane/workspace.ts` local wrappers.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy ustcode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy ustcode database wrapper.
...
All migration groups are complete or superseded. `packages/ustcode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No ustcode data-migration group remains.

The previous ustcode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate ustcode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/ustcode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/ustcode/src`
- `bun typecheck` from `packages/ustcode`
- Relevant package tests from `packages/ustcode`, not the repo root
