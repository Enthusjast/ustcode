...
Move the canonical UstCode terminal application from
`packages/ustcode/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/ustcode ---\
                      > @enthusjast/ustcode-tui -> @enthusjast/ustcode-sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/ustcode`,
`packages/cli`, or `@enthusjast/ustcode-core`.

The SDK is the TUI's UstCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/ustcode`.
- Do not replace private `packages/ustcode` imports with `@enthusjast/ustcode-core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@enthusjast/ustcode-tui`.

## Ownership Boundary

### `@enthusjast/ustcode-tui` Owns
...
- UstCode domain data displayed by the TUI
...
```text
packages/ustcode/src/cli/cmd/tui
```

Its private dependency on `packages/ustcode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/ustcode/src/*`.
...
Status: Completed. The private `@enthusjast/ustcode-tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@enthusjast/ustcode-tui`.
...
- The package has no dependency on `ustcode`, `@enthusjast/ustcode-cli`, or
  `@enthusjast/ustcode-core`.
...
primitives now live in `@enthusjast/ustcode-tui`. The legacy host consumes explicit
...
and build inputs through `@enthusjast/ustcode-tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads UstCode globals or process state;
...
Replace process-global UstCode state with resolved TUI inputs.
...
`@enthusjast/ustcode-tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/ustcode` initially.
...
canonical in `@enthusjast/ustcode-tui`. Configured references resolve through the new
...
adapters rather than UstCode domain implementations.

Make the SDK the only UstCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports UstCode data and operations only from
  `@enthusjast/ustcode-sdk`.
...
`@enthusjast/ustcode-tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/ustcode`.
...
- The package has no imports from `packages/ustcode`, `packages/cli`, or
  `@enthusjast/ustcode-core`.
...
public `@enthusjast/ustcode-tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/ustcode`.
- Keep the legacy embedded worker and server startup in `packages/ustcode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@enthusjast/ustcode-tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/ustcode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@enthusjast/ustcode-tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@enthusjast/ustcode-core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for UstCode domain state.
...
cd packages/ustcode && bun typecheck
...
rg '@enthusjast/ustcode-core|packages/ustcode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/ustcode/src packages/ustcode/test
```
...
- Build the current-platform `packages/ustcode` binary.
