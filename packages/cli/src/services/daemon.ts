...
import { createUstcodeClient } from "@enthusjast/ustcode-sdk/v2/client"
import { ServerAuth } from "@enthusjast/ustcode-server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createUstcodeClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@ustcode/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createUstcodeClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createUstcodeClient({ baseUrl: connection.url, headers: connection.headers })
    })
