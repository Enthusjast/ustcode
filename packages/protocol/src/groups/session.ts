...
import { PromptInput } from "@enthusjast/ustcode-schema/prompt-input"
import { Session } from "@enthusjast/ustcode-schema/session"
import { Project } from "@enthusjast/ustcode-schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@enthusjast/ustcode-schema/schema"
...
import { SessionEvent } from "@enthusjast/ustcode-schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this UstCode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this UstCode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
