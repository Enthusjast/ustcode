...
const compatibleChatRoute = OpenAICompatibleChat.route
...
describe("llm constructors", () => {
  ...
  test("updates canonical models from the model datatype", () => {
    ...
    expect(updated.route).toBe(compatibleChatRoute)
    expect(updated.defaults?.generation).toEqual({ maxTokens: 20 })
    expect(updated.compatibility).toEqual({ toolSchema: "moonshot" })
