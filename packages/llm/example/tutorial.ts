 ...
 *
 * Run from `packages/llm` with an API key for an OpenAI-compatible endpoint in
 * the environment:
 *
...
// 1. Pick a model. The openai-compatible provider records provider identity,
// protocol choice, capabilities, deployment options, authentication, and
// defaults. Point `baseURL` at any OpenAI Chat-compatible endpoint.
...
//     OpenAI cache keys and store behavior.
