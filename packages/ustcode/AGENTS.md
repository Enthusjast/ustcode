...
- Running `bun dev` from `packages/ustcode` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s ustcode-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt ustcode-dev`.
- Stop the session explicitly when done: `tmux kill-session -t ustcode-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@ustcode/Foo") {}
