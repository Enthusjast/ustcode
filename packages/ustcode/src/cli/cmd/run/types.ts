// Shared type vocabulary for the direct interactive mode (`ustcode --mini`).
//
...
import type { UstcodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@enthusjast/ustcode-sdk/v2"
import type { TuiConfig } from "@enthusjast/ustcode-tui/config"
...
type PromptModel = Parameters<UstcodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<UstcodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<UstcodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<UstcodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<UstcodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<UstcodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<UstcodeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<UstcodeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<UstcodeClient["question"]["reject"]>[0]
