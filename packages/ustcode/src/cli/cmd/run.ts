import type { PermissionV1 } from "@enthusjast/ustcode-core/v1/permission"
import { FSUtil } from "@enthusjast/ustcode-core/fs-util"
// CLI entry point for `ustcode run` and `ustcode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`ustcode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`ustcode --mini --attach`): connects to a running
//      ustcode server and runs interactive mode against it.
//
...
import { createUstcodeClient, type UstcodeClient, type ToolPart } from "@enthusjast/ustcode-sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<UstcodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run ustcode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running ustcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to USTCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to USTCODE_SERVER_USERNAME or 'ustcode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: UstcodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function current(sdk: UstcodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: UstcodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: UstcodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: UstcodeClient) {
        ...
        async function loop(client: UstcodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createUstcodeClient({
        baseUrl: "http://ustcode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
