...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall ustcode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall UstCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @enthusjast/ustcode",
      pnpm: "pnpm uninstall -g @enthusjast/ustcode",
      bun: "bun remove -g @enthusjast/ustcode",
      yarn: "yarn global remove @enthusjast/ustcode",
      brew: "brew uninstall ustcode",
      choco: "choco uninstall ustcode",
      scoop: "scoop uninstall ustcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "ustcode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using UstCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# ustcode") || content.includes(".ustcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".ustcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".ustcode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".ustcode"))
    ) {
      continue
    }
