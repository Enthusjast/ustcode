...
import { serviceUse } from "@enthusjast/ustcode-core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@enthusjast/ustcode-core/installation/version"
...
import { EffectFlock } from "@enthusjast/ustcode-core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@enthusjast/ustcode-core/v1/config/config"
import { RemoteAuthError } from "@enthusjast/ustcode-core/v1/config/error"
import { ConfigPermissionV1 } from "@enthusjast/ustcode-core/v1/config/permission"
...
// Canonical location of the config JSON Schema. Hosted via GitHub Pages; see config-schema.json at repo root.
const CONFIG_SCHEMA_URL = "https://ustcode.enthusjast.cc/config.json"
...
export class Service extends Context.Service<Service, Interface>()("@ustcode/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.USTCODE_CONFIG && !Flag.USTCODE_CONFIG_DIR && !Flag.USTCODE_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: CONFIG_SCHEMA_URL }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "ustcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "ustcode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "USTCODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/ustcode`
        ...
        if (Flag.USTCODE_CONFIG) {
          yield* merge(Flag.USTCODE_CONFIG, yield* loadFile(Flag.USTCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.USTCODE_CONFIG })
        }

        if (!Flag.USTCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("ustcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.USTCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from USTCODE_CONFIG_DIR", { path: Flag.USTCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".ustcode") || dir === Flag.USTCODE_CONFIG_DIR) {
            for (const file of ["ustcode.json", "ustcode.jsonc"]) {
          ...
          const dep = yield* npmSvc
            .install(dir, {
              add: [
                {
                  name: "@enthusjast/ustcode-plugin",
          ...
          // Auto-discovered plugins under `.ustcode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.USTCODE_CONFIG_CONTENT) {
          const source = "USTCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.USTCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from USTCODE_CONFIG_CONTENT")
        }
        ...
        if (Flag.USTCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.USTCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("USTCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.USTCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.USTCODE_DISABLE_PRUNE) {
