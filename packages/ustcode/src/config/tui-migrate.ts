...
const TUI_SCHEMA_URL = "https://ustcode.enthusjast.cc/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from ustcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const ustcode = await ustcodeFiles(input)
  for (const file of ustcode) {
...
async function ustcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "ustcode"),
    ...(await Filesystem.findUp(["ustcode.json", "ustcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "ustcode"))
  }
  if (Flag.USTCODE_CONFIG) files.push(Flag.USTCODE_CONFIG)
