...
import { isRecord } from "@enthusjast/ustcode-tui/util/record"
...
import { TuiKeybind } from "@enthusjast/ustcode-tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@ustcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old ustcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.ustcode`
  // folders between cwd and home, and USTCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.USTCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit USTCODE_TUI_CONFIG override, if set.
  if (Flag.USTCODE_TUI_CONFIG) {
    const configFile = Flag.USTCODE_TUI_CONFIG
  ...
  // 4. `.ustcode` directories (and USTCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".ustcode") || dir === Flag.USTCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".ustcode") && dir !== Flag.USTCODE_CONFIG_DIR) continue
