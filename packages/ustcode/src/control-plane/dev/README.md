This is a plugin to simulate a remote environment locally. Add this to `.ustcode/ustcode.jsonc`:

```json
  "plugin": ["../packages/ustcode/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate UstCode server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/ustcode/script/run-workspace-server
```

With the plugin install, you can now run UstCode and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
