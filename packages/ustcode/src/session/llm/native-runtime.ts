...
import type { LLMClientShape } from "@enthusjast/ustcode-llm/route"
...
export function status(input: Pick<StreamInput, "model" | "provider" | "auth">): RuntimeStatus {
  return statusWithFetch(input)
}

function statusWithFetch(input: Pick<StreamInput, "model" | "provider" | "auth">): RuntimeStatus {
  const npm = input.model.api.npm
  if (npm !== "@ai-sdk/openai-compatible")
    return { type: "unsupported", reason: "provider package is not OpenAI-compatible" }
...
export function stream(input: StreamInput): StreamResult {
  const current = statusWithFetch(input)
  if (current.type === "unsupported") return current

  // Integration point with @enthusjast/ustcode-llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains ustcode-owned. The native runtime only adapts
      // the @enthusjast/ustcode-llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
