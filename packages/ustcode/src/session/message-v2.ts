...
import {
  ...
  User,
  WithParts,
} from "@enthusjast/ustcode-core/v1/session"
...
import { MessageTable, PartTable, SessionTable } from "@enthusjast/ustcode-core/session/sql"
...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  // (images, PDFs) inside tool results must be extracted and injected as user messages.
      ...
      for (const part of msg.parts) {
        ...
        if (part.type === "tool") {
          ...
          if (part.state.status === "completed") {
            ...
            // OpenAI-compatible tool results only support string content, so extract
            // media files (images, PDFs) to be sent as a separate user message
            const mediaAttachments = attachments.filter((a) => isMedia(a.mime))
            if (mediaAttachments.length > 0) {
              media.push(...mediaAttachments)
            }
            const finalAttachments = attachments.filter((a) => !isMedia(a.mime))
