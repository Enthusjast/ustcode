...
import { UstCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for ustcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(UstCodeHttpApi)
...
export const PublicApi = UstCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "ustcode",
    version: "1.0.0",
    description: "ustcode api",
