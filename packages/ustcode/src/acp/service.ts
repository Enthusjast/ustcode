...
import type { AssistantMessage, Message, UstcodeClient, SessionMessageResponse } from "@enthusjast/ustcode-sdk/v2"
...
export const AuthMethodID = "ustcode-login"
...
export class Service extends Context.Service<Service, Interface>()("@ustcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `ustcode auth login` in the terminal",
      name: "Login with ustcode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: UstcodeClient) {
...
function makeUsageService(sdk: UstcodeClient) {
...
async function loadDirectorySnapshot(sdk: UstcodeClient, directory: string) {
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "UstCode service failure", service })
}
