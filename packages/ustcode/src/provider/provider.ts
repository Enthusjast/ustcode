...
import { optional } from "@enthusjast/ustcode-core/schema"
...
export class Service extends Context.Service<Service, Interface>()("@ustcode/Provider") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      const priority = smallModelFamilyPriority
      const models = sortBy(
...
export function sort<T extends { id: string; default?: boolean }>(models: T[]) {
  return sortBy(
    models,
    [(model) => (model.default ? 0 : 1), "asc"],
