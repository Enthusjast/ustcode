...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@enthusjast/ustcode-plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@enthusjast/ustcode-core/effect/app-node-builder"
import { errorData, errorMessage } from "@enthusjast/ustcode-tui/util/error"
...
import { hasTheme, upsertTheme } from "@enthusjast/ustcode-tui/context/theme"
import { Global } from "@enthusjast/ustcode-core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@enthusjast/ustcode-core/util/flock"
import { Flag } from "@enthusjast/ustcode-core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@enthusjast/ustcode-tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@enthusjast/ustcode-core/v1/config/plugin"
import { createCommandShim } from "@enthusjast/ustcode-tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@enthusjast/ustcode-tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".ustcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".ustcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".ustcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.USTCODE_PURE ? [] : pluginOrigins
    if (Flag.USTCODE_PURE && pluginOrigins.length) {
    }
