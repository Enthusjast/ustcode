...
export async function checkPluginCompatibility(target: string, ustcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(ustcodeVersion) || semver.major(ustcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.ustcode
  if (typeof range !== "string") return
  if (!semver.satisfies(ustcodeVersion, range)) {
    throw new Error(`Plugin requires ustcode ${range} but running ${ustcodeVersion}`)
  }
}
