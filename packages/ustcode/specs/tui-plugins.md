...
- Author package entrypoint is `@enthusjast/ustcode-plugin/tui`.
...
```json
{
  "$schema": "https://ustcode.enthusjast.cc/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/ustcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@enthusjast/ustcode-plugin/tui`.
- `@enthusjast/ustcode-plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@enthusjast/ustcode-plugin/tui"
...
- The value is a semver range checked against the running UstCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.ustcode` is absent, no check is performed (backward compatible).
...
- `ustcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `ustcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.ustcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.ustcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.ustcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.ustcode` dir or `USTCODE_CONFIG_DIR`, UstCode installs `@enthusjast/ustcode-plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@enthusjast/ustcode-plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus UstCode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
UstCode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, UstCode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@enthusjast/ustcode-plugin/tui`.
...
- `message` is required; `title` defaults to `"ustcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `ustcode.default`.
...
- Local plugins persist installed themes under the local `.ustcode/themes` area near the plugin config source.
...
- `--pure` / `USTCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.ustcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.ustcode/plugins/tui-vim.tsx`
- Local smoke config: `.ustcode/tui.json`
- Local smoke theme: `.ustcode/plugins/smoke-theme.json`
