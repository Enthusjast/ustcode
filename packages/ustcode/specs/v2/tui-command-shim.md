...
Delete `packages/ustcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/ustcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/ustcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/ustcode`
- TUI plugin loader tests in `packages/ustcode` if runtime plugin API wiring changed
