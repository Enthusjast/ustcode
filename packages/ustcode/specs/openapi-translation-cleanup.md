...
Trim `packages/ustcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/ustcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/ustcode`.
- `[x]` `bun typecheck` from `packages/ustcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/ustcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/ustcode`.
- `[x]` `bun dev generate > /tmp/ustcode-openapi.json` from `packages/ustcode`.
...
- Edit `packages/ustcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/ustcode-openapi.json` from `packages/ustcode`.
...
- Check source schemas in files like `packages/ustcode/src/session/schema.ts`, `packages/ustcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/ustcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/ustcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
