...
Practical reference for a future `packages/server` split after the ustcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/ustcode`.
...
Until enough shared service code lives outside `packages/ustcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/ustcode`

It should not import `packages/ustcode` services while `packages/ustcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on ustcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/ustcode` implementation details.
