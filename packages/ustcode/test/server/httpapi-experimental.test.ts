...
import { AccountV2 } from "@enthusjast/ustcode-core/account"
import { AccountTable } from "@enthusjast/ustcode-core/account/sql"
...
function withCreatedWorktree(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      expect(info).toMatchObject({ name, branch: "ustcode/api-test" })
    ...
    () =>
      Effect.gen(function* () {
        ...
        const [toolList, toolIDs, worktrees, resources] = yield* Effect.all(
          [
            request(`${ExperimentalPaths.tool}?provider=ustcode&model=gpt-5`, directory),
