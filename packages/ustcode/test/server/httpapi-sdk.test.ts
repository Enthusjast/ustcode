...
import { createUstcodeClient } from "@enthusjast/ustcode-sdk/v2"
...
type Sdk = ReturnType<typeof createUstcodeClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.USTCODE_SERVER_PASSWORD = input?.password
      Flag.USTCODE_SERVER_USERNAME = input?.username
      // Bun's fetch rejects 0.0.0.0 with 502; the server binds all interfaces, so
      // target the loopback address explicitly.
      const baseUrl = HttpServer.formatAddress(server.address).replace("0.0.0.0", "127.0.0.1")
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".ustcode", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-ustcode-directory")).toBe(false)
        expect(request!.headers.has("x-ustcode-workspace")).toBe(false)
      }),
    ),
  )
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        const badSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("ustcode", "wrong") },
        })
        const bad = yield* capture(() => badSdk.file.read({ path: "hello.txt" }))
        const goodSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("ustcode", "secret") },
        })
