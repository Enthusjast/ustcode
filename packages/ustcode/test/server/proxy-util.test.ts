...
describe("ProxyUtil", () => {
  ...
  describe("headers", () => {
    ...
    test("strips ustcode-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          "x-ustcode-directory": "/home/user/project",
      ...
      const result = ProxyUtil.headers(req)
      expect(result.get("x-ustcode-directory")).toBeNull()
      expect(result.get("x-ustcode-workspace")).toBeNull()
