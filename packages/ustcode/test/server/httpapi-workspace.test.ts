...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-ustcode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-ustcode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://ustcode.enthusjast.cc/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-ustcode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-ustcode-workspace")
