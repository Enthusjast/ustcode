...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".ustcode")])),
})
...
describe("tool.registry", () => {
  ...
  withCodeMode.instance("exposes execute when code mode is enabled", () =>
    Effect.gen(function* () {
      ...
      const tools = yield* registry.tools({
        providerID: ProviderV2.ID.ustcode,
  ...
  it.instance("loads tools from .ustcode/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const ustcode = path.join(test.directory, ".ustcode")
      const tool = path.join(ustcode, "tool")
  ...
  it.instance("ignores non-tool exports in .ustcode/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".ustcode", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — a plugin was registering a tool
  ...
  it.instance("loads tools from .ustcode/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const ustcode = path.join(test.directory, ".ustcode")
      const tools = path.join(ustcode, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".ustcode", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const ustcode = path.join(test.directory, ".ustcode")
        const customTools = path.join(ustcode, "tools")
        const plugin = path.join(ustcode, "node_modules", "@enthusjast", "ustcode-plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(ustcode, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@enthusjast/ustcode-plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@enthusjast/ustcode-plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".ustcode", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(ustcode, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@enthusjast/ustcode-plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(ustcode, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(ustcode, "node_modules", "cowsay")
