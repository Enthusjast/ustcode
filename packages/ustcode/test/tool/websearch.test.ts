...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.USTCODE_WEBSEARCH_PROVIDER

    try {
      process.env.USTCODE_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.USTCODE_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.USTCODE_WEBSEARCH_PROVIDER
      else process.env.USTCODE_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is enabled for USTC or explicit websearch provider flags", () => {
    ...
    expect(webSearchEnabled(ProviderV2.ID.make("openai"), { exa: false, parallel: true })).toBe(true)
  })
