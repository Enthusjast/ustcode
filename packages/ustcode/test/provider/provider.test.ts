...
it.instance("provider loaded from env variable", () =>
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("anthropic")].source).toBe("env")
  }),
)
...
it.instance(
  ...
  Effect.gen(function* () {
    yield* setProcessEnv("ANTHROPIC_API_KEY", "test-api-key")
    const providers = yield* list
    expect(providers[ProviderV2.ID.make("anthropic")]).toBeUndefined()
  }),
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const providers = yield* list
    expect(providers[ProviderV2.ID.make("anthropic")]).toBeDefined()
    const models = Object.keys(providers[ProviderV2.ID.make("anthropic")].models)
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const providers = yield* list
    expect(providers[ProviderV2.ID.make("anthropic")]).toBeDefined()
    expect(providers[ProviderV2.ID.make("anthropic")].models["my-alias"]).toBeDefined()
    expect(providers[ProviderV2.ID.make("anthropic")].models["my-alias"].name).toBe("My Custom Alias")
  }),
  {
    config: {
      provider: {
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("anthropic")].options.timeout).toBe(60000)
    expect(providers[ProviderV2.ID.make("anthropic")].options.headerTimeout).toBe(10000)
    expect(providers[ProviderV2.ID.make("anthropic")].options.chunkTimeout).toBe(15000)
  }),
...
it.instance("getModel returns model for valid provider/model", () =>
  Effect.gen(function* () {
    ...
    const model = yield* provider.getModel(ProviderV2.ID.make("anthropic"), ModelV2.ID.make("claude-sonnet-4-6"))
...
it.instance("getModel throws ModelNotFoundError for invalid model", () =>
  Effect.gen(function* () {
    ...
    const exit = yield* Provider.use
      .getModel(ProviderV2.ID.make("anthropic"), ModelV2.ID.make("nonexistent-model"))
      .pipe(Effect.exit)
...
it.instance(
  ...
  Effect.gen(function* () {
    const providers = yield* list
    const model = providers[ProviderV2.ID.make("anthropic")].models["claude-sonnet-4-6"]
...
it.instance("closest finds model by partial match", () =>
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    const result = yield* Provider.use.closest(ProviderV2.ID.make("anthropic"), ["sonnet-4"])
...
it.instance(
  ...
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    const providers = yield* list
    expect(providers[ProviderV2.ID.make("anthropic")].models["my-sonnet"]).toBeDefined()

    const model = yield* Provider.use.getModel(ProviderV2.ID.make("anthropic"), ModelV2.ID.make("my-sonnet"))
    expect(model).toBeDefined()
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const providers = yield* list
    const model = providers[ProviderV2.ID.make("openai")].models["custom-gpt-chat"]
...
it.instance("getSmallModel returns appropriate small model", () =>
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    const model = yield* Provider.use.getSmallModel(ProviderV2.ID.make("anthropic"))
    expect(model).toBeDefined()
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const providers = yield* list
    expect(providers[ProviderV2.ID.make("anthropic")]).toBeDefined()
    expect(providers[ProviderV2.ID.make("openai")]).toBeDefined()
    expect(providers[ProviderV2.ID.make("anthropic")].options.timeout).toBe(30000)
    expect(providers[ProviderV2.ID.make("openai")].options.timeout).toBe(60000)
  }),
  {
    config: {
      provider: {
...
it.instance(
  ...
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    const providers = yield* list
    expect(providers[ProviderV2.ID.make("anthropic")].models["sonnet"].name).toBe("sonnet")
  }),
  {
    config: {
      provider: {
...
it.instance(
  ...
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    const providers = yield* list
    const model = providers[ProviderV2.ID.make("anthropic")].models["claude-sonnet-4-20250514"]
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("google")]).toBeUndefined()
  }),
  {
...
it.instance("getModel returns consistent results", () =>
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    const model1 = yield* Provider.use.getModel(ProviderV2.ID.make("anthropic"), ModelV2.ID.make("claude-sonnet-4-6"))
    const model2 = yield* Provider.use.getModel(ProviderV2.ID.make("anthropic"), ModelV2.ID.make("claude-sonnet-4-6"))
...
it.instance("ModelNotFoundError includes suggestions for typos", () =>
  Effect.gen(function* () {
    ...
    const error = yield* Provider.use
      .getModel(ProviderV2.ID.make("anthropic"), ModelV2.ID.make("claude-sonet-4"))
      .pipe(Effect.flip)
...
it.instance("ModelNotFoundError suggests catalog models for unloaded providers", () =>
  Effect.gen(function* () {
    yield* remove("USTCODE_API_KEY")
    const error = yield* Provider.use
      .getModel(ProviderV2.ID.make("ustcode"), ModelV2.ID.make("claude-haiku-fake-model"))
      .pipe(Effect.flip)
...
it.instance("getProvider returns provider info", () =>
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    const provider = yield* Provider.use.getProvider(ProviderV2.ID.make("anthropic"))
...
it.instance("closest returns undefined when no partial match found", () =>
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    const result = yield* Provider.use.closest(ProviderV2.ID.make("anthropic"), ["nonexistent-xyz-model"])
    expect(result).toBeUndefined()
  }),
)
...
it.instance("closest checks multiple query terms in order", () =>
  Effect.gen(function* () {
    ...
    const result = yield* Provider.use.closest(ProviderV2.ID.make("anthropic"), ["nonexistent", "haiku"])
    expect(result).toBeDefined()
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("anthropic")].options.timeout).toBe(30000)
    expect(providers[ProviderV2.ID.make("anthropic")].options.headers["X-Custom"]).toBe("custom-value")
  }),
  {
    config: {
...
it.instance(
  ...
  Effect.gen(function* () {
    yield* set("OPENAI_API_KEY", "test-api-key")
    const providers = yield* list
    const model = providers[ProviderV2.ID.make("openai")].models["my-custom-model"]
    expect(model).toBeDefined()
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const providers = yield* list
    expect(providers[ProviderV2.ID.make("openrouter")]).toBeDefined()

    // New model not in database should inherit api.url from provider
    const intellect = providers[ProviderV2.ID.make("openrouter")].models["prime-intellect/intellect-3"]
    ...
    const deepseek = providers[ProviderV2.ID.make("openrouter")].models["deepseek/deepseek-r1-0528"]
...
test("mode options and cost are derived from the base model", () => {
  ...
  expect(pro.options).toEqual({ reasoning: { mode: "pro" }, serviceTier: "priority" })
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const model = providers[ProviderV2.ID.make("test-provider")].models["reasoning-model"]
    expect(model.variants!["medium"]).toBeDefined()
    // Should have both the generated reasoning effort and the custom option
    expect(model.variants!["medium"].reasoningEffort).toBe("medium")
    expect(model.variants!["medium"].extraOption).toBe("custom-value")
  }),
  {
    config: {
      provider: {
        "test-provider": {
          ...
          models: {
            "reasoning-model": {
              ...
              tool_call: true,
              limit: { context: 128000, output: 16000 },
              variants: { medium: { extraOption: "custom-value" } },
            },
          },
          options: { apiKey: "test-key" },
        },
...
it.instance(
  ...
  Effect.gen(function* () {
    const providers = yield* list
    const model = providers[ProviderV2.ID.make("cortecs")].models["deepseek-v4-flash"]
    expect(model.variants).toBeDefined()
    expect(model.variants!["high"]).toBeUndefined()
  ...
  {
    config: {
      provider: { cortecs: { models: { "deepseek-v4-flash": { variants: { high: { disabled: true } } } } } },
    },
  },
)
...
it.instance(
  ...
  {
    config: {
      provider: { "cloudflare-ai-gateway": { options: { metadata: { invoked_by: "test", project: "ustcode" } } } },
    },
  },
)
...
it.effect("plugin config providers persist after instance dispose", () =>
  Effect.gen(function* () {
    ...
    const configDir = path.join(dir, ".ustcode")
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".ustcode")
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() =>
