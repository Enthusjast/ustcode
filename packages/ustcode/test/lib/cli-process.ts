// Subprocess test harness for the ustcode CLI. Spawns the real binary against
...
//
// Configuration flows through ustcode's built-in test affordances:
//   - USTCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - USTCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - USTCODE_DISABLE_PROJECT_CONFIG : skip walking up for ustcode.json
//   - USTCODE_PURE                : skip external plugin discovery + install
//   - USTCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `ustcode.run` is fully wired. The shape supports adding more
// builders (`ustcode.serve(opts)`, `ustcode.acp(opts)`, `ustcode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of UstcodeCli.
...
import { AppProcess } from "@enthusjast/ustcode-core/process"
...
const ustcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(ustcodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    USTCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    USTCODE_CONFIG_CONTENT: configJson,
    USTCODE_DISABLE_PROJECT_CONFIG: "1",
    USTCODE_PURE: "1",
    USTCODE_DISABLE_AUTOUPDATE: "1",
    USTCODE_DISABLE_AUTOCOMPACT: "1",
    USTCODE_DISABLE_MODELS_FETCH: "1",
    USTCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `ustcode run`. New flags should
...
// `ustcode serve` is a long-lived process — it never exits on its own.
...
// `ustcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type UstcodeCli = {
  ...
  // Spawn `ustcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `ustcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("ustcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          USTCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("ustcode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("ustcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "ustcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `ustcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("ustcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const ustcode: UstcodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, ustcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `ustcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "ustcode") {
...
// every call site. Use this for any test that needs the ustcode CLI fixture.
//
...
// resources (e.g. `ustcode.serve`) without an extra `Effect.scoped` wrapper —
