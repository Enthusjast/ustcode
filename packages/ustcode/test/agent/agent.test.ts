...
it.instance("plan agent denies edits except .ustcode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".ustcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".ustcode", "skill", "perm-skill")
...
`,
      ...
      process.env.USTCODE_TEST_HOME = test.directory
