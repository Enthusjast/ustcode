...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "ustcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "ustcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ustcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ustcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["ustcode.json", "ustcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "ustcode.json"),
        path.join(project, "ustcode.jsonc"),
      ])
    })
  })
