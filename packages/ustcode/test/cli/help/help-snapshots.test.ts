...
// terminal sizes. The default ustcode tui command is excluded —
// `ustcode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `ustcode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real ustcode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("ustcode CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ ustcode }) =>
      Effect.gen(function* () {
        const topLevel = yield* ustcode.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* ustcode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`ustcode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`ustcode ${argv.join(" ")} --help`)
        }
