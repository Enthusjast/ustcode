...
describe("ustcode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // USTCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ ustcode }) =>
      Effect.gen(function* () {
        const r = yield* ustcode.spawn(["mcp", "list"])
        ustcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ ustcode }) =>
      Effect.gen(function* () {
        const r = yield* ustcode.spawn(["providers", "list"])
        ustcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ ustcode }) =>
      Effect.gen(function* () {
        const r = yield* ustcode.spawn(["models"])
        ustcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ ustcode }) =>
      Effect.gen(function* () {
        const r = yield* ustcode.spawn(["agent", "list"])
        ustcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh USTCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ ustcode }) =>
      Effect.gen(function* () {
        const r = yield* ustcode.spawn(["session", "list"])
        ustcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ ustcode }) =>
      Effect.gen(function* () {
        const r = yield* ustcode.spawn(["stats"])
        ustcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under USTCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ ustcode }) =>
      Effect.gen(function* () {
        const r = yield* ustcode.spawn(["db", "path"])
        ustcode.expectExit(r, 0, "db path")
