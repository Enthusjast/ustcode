...
import type { QuestionRequest } from "@enthusjast/ustcode-sdk/v2"
import { UstcodeKeymapProvider, registerUstcodeKeymap } from "@enthusjast/ustcode-tui/keymap"
import {
...
function model(input: {
  ...
  return {
    ...
    api: {
      id: "ustcode",
      url: "https://ustcode.enthusjast.cc",
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerUstcodeKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerUstcodeKeymap(keymap, renderer, tuiConfig)
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "ustcode", modelID: "gpt-5" },
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerUstcodeKeymap(keymap, renderer, tuiConfig)
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "ustcode", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("ustcode")
    expect(frame).toContain("GPT-5")
