...
import { UstcodeClient, type GlobalEvent } from "@enthusjast/ustcode-sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<UstcodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<UstcodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<UstcodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<UstcodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<UstcodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://ustcode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: UstcodeClient["event"]["subscribe"]
    globalEvent?: UstcodeClient["global"]["event"]
    promptAsync?: UstcodeClient["session"]["promptAsync"]
    status?: UstcodeClient["session"]["status"]
    messages?: UstcodeClient["session"]["messages"]
    children?: UstcodeClient["session"]["children"]
    permissions?: UstcodeClient["permission"]["list"]
    questions?: UstcodeClient["question"]["list"]
  } = {},
) {
  const client = new UstcodeClient()

  const subscribe: UstcodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: UstcodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: UstcodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: UstcodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: UstcodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: UstcodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: UstcodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: UstcodeClient["question"]["list"] = input.questions ?? (() => ok([]))
