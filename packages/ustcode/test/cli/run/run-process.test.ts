// Subprocess integration tests for `ustcode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `ustcode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `USTCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("ustcode run (non-interactive subprocess)", () => {
  ...
  cliIt.live(
    ...
    ({ llm, ustcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* ustcode.run("say hi")
        ustcode.expectExit(result, 0)
  ...
  cliIt.live(
    ...
    ({ llm, ustcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* ustcode.run("use a tool", {
  ...
  cliIt.live(
    ...
    ({ llm, ustcode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* ustcode.run("think", { extraArgs: ["--thinking"] })
        ustcode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* ustcode.run("think again")
        ustcode.expectExit(plain, 0)
  ...
  cliIt.live(
    ...
    ({ llm, ustcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* ustcode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.live(
    ...
    ({ llm, ustcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* ustcode.run("say hi", { format: "json" })
        ustcode.expectExit(result, 0)

        const events = ustcode.parseJsonEvents(result.stdout)
  ...
  cliIt.live(
    ...
    ({ ustcode }) =>
      Effect.gen(function* () {
        const result = yield* ustcode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.live(
    ...
    ({ llm, ustcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* ustcode.run("exercise json records", {
          format: "json",
  ...
  cliIt.live(
    ...
    ({ llm, ustcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* ustcode.run("fail after output", { format: "json" })
  ...
  cliIt.live(
    ...
    ({ home, llm, ustcode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* ustcode.run("request permission", { permission: { bash: "ask" } })
        ustcode.expectExit(denied, 0)
        ...
        const allowed = yield* ustcode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        ustcode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* ustcode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        ustcode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, ustcode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* ustcode.serve()

        const result = yield* ustcode.run("read the attachment", {
  ...
  cliIt.live(
    ...
    ({ home, ustcode }) =>
      Effect.gen(function* () {
        const result = yield* ustcode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, ustcode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* ustcode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
