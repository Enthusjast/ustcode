...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ ustcode }) =>
    Effect.gen(function* () {
      const result = yield* ustcode.spawn(["--replay-limit", "10"])

      ustcode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ ustcode }) =>
    Effect.gen(function* () {
      const result = yield* ustcode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ ustcode }) =>
    Effect.gen(function* () {
      const result = yield* ustcode.spawn(["--mini", "--port", "4096"])
