...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/ustcode.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/ustcode.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/ustcode.jsonc", dir: ".ustcode", suggestion: "ustcode" },
        expected:
          'Directory ".ustcode" in /tmp/ustcode.jsonc is not valid. Rename the directory to "ustcode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/ustcode.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/ustcode.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `ustcode models` to list available models",
      "Or check your config (ustcode.json) provider/model names",
    ].join("\n")
