...
describe("ustcode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ ustcode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ ustcode }))
        ...
        expect(initialized.agentInfo?.name).toBe("UstCode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ ustcode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ ustcode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("ustcode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "ustcode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.USTCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ ustcode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("ustcode-login")
