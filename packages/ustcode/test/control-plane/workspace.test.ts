...
import { EventSequenceTable } from "@enthusjast/ustcode-core/event/sql"
...
import { WorkspaceV2 } from "@enthusjast/ustcode-core/workspace"
import { WorkspaceTable } from "@enthusjast/ustcode-core/control-plane/workspace.sql"
...
const originalEnv = {
  USTCODE_AUTH_CONTENT: process.env.USTCODE_AUTH_CONTENT,
  USTCODE_EXPERIMENTAL_WORKSPACES: process.env.USTCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.USTCODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@ustcode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.USTCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=ustcode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.USTCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.USTCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.USTCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=ustcode-test")
