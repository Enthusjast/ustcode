...
import type { Event, UstcodeClient } from "@enthusjast/ustcode-sdk/v2"
...
type PermissionReplyParams = Parameters<UstcodeClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "ustcode-acp-permission-"))
