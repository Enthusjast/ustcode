...
import { Hash } from "@enthusjast/ustcode-core/util/hash"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const ustcodeFile = path.join(tmp, ".git", "ustcode")
      expect(yield* Effect.promise(() => Bun.file(ustcodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "ustcode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "ustcode")
      const wrongCache = path.join(parentDir, ".git", "ustcode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "ustcode")
      const cacheB = path.join(bareB, "ustcode")
      const wrongCache = path.join(parentDir, ".git", "ustcode")
