...
import type { NamedError } from "@enthusjast/ustcode-core/util/error"
...
describe("session.retry.retryable", () => {
  ...
  test("maps free usage limits to a retry action", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
    ...
    expect(SessionRetry.retryable(error, "ustcode")).toEqual({
      message: "Free usage limit reached",
      action: {
        reason: "free_tier_limit",
        provider: "ustcode",
        title: "Free limit reached",
        message: "Free usage limit reached. Please try again later or switch to a different model.",
        label: "retry",
      },
    })
  })

  test("maps usage limits to a retry action", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
    ...
    expect(SessionRetry.retryable(error, "ustcode-go")).toEqual({
  ...
  test("maps usage limits without limit metadata", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
        message: "Subscription quota exceeded. You can continue using free models.",
        isRetryable: true,
    ...
    expect(SessionRetry.retryable(error, "ustcode-go")?.action?.message).toBe(
      "Usage limit reached. It will reset in 15 minutes.",
    )
  })
})
