...
import { HttpRecorder } from "@enthusjast/ustcode-http-recorder"
import { HttpRecorderInternal } from "@enthusjast/ustcode-http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@enthusjast/ustcode-llm"
import { RequestExecutor } from "@enthusjast/ustcode-llm/route"
...
const zenURL = (connection: string) => `https://console.ustcode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.USTCODE_RECORD_OPENAI_AUTH
...
// Recorded native-runtime scenarios require cassettes recorded against the
// bundled third-party providers (OpenAI, Anthropic, UstCode Zen). All of those
// providers were removed, so no scenarios remain runnable.
...
const selectedScenarios = new Set(
  (envValue("USTCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set USTCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the ustcode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://ustcode.enthusjast.cc/config.json", ...scenario.config(model) }),
    ),
  )
