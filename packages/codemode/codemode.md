...
This is the living design and status document for `@enthusjast/ustcode-codemode` and its existing V2 UstCode adapter.
...
`packages/ustcode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
