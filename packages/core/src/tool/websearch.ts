...
export class ConfigService extends Context.Service<ConfigService, Config>()("@ustcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.USTCODE_WEBSEARCH_PROVIDER === "exa" || process.env.USTCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.USTCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("USTCODE_EXPERIMENTAL") || truthy("USTCODE_ENABLE_EXA") || truthy("USTCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("USTCODE_ENABLE_PARALLEL") || truthy("USTCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `ustcode/${InstallationVersion}`,
