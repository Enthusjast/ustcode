...
// etc.).
//
// The visual language mirrors the UstCode app: the design tokens are a curated
// subset of the OC-2 semantic tokens in the app's theme CSS, and the wordmark
// matches the app logo. Keep this file in sync when the brand changes.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `UstCode is now connected to ${escapeHtml(provider)}.` : "UstCode is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `UstCode couldn't finish connecting to ${escapeHtml(provider)}.`
        : "UstCode couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from UstCode.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · UstCode</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("UstCode couldn't finish connecting to "+PROVIDER+"."):"UstCode couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from UstCode."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("UstCode is now connected to "+PROVIDER+"."):"UstCode is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// Curated subset of OC-2 tokens (from the app theme CSS). Default is light;
// dark applies via prefers-color-scheme. The [data-theme] selectors let a host
// force a scheme without changing the default.
const LIGHT_VARS = `
    --oc-bg: #f8f8f8;
    --oc-card: #fcfcfc;
...
// UstCode wordmark — same path geometry as the app logo.
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="UstCode" role="img">
