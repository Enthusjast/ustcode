...
import { ConfigSkillPlugin } from "@enthusjast/ustcode-core/config/plugin/skill"
...
describe("ConfigSkillPlugin.Plugin", () => {
  it.effect("registers configured skill directories and URLs", () =>
    Effect.gen(function* () {
      const directory = AbsolutePath.make("/repo/packages/desktop")
      ...
      yield* ConfigSkillPlugin.Plugin.effect(
        ...
        Effect.provideService(
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Directory({ type: "directory", path: AbsolutePath.make("/repo/.ustcode") }),
                new Config.Document({
                  type: "document",
                  info: decode({
      ...
      expect(sources).toEqual([
        SkillV2.DirectorySource.make({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.ustcode", "skill")),
        }),
        SkillV2.DirectorySource.make({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.ustcode", "skills")),
        }),
