...
import { ConfigAgentPlugin } from "@enthusjast/ustcode-core/config/plugin/agent"
...
describe("ConfigAgentPlugin.Plugin", () => {
  it.effect("matches POSIX paths against home-relative permissions", () =>
    Effect.gen(function* () {
      ...
      expect(PermissionV2.evaluate("external_directory", "/home/test/p/ustcode/src/*", permissions).effect).toBe(
        "allow",
      )
  ...
  it.effect("matches Windows paths against home-relative permissions", () =>
    Effect.gen(function* () {
      ...
      expect(
        PermissionV2.evaluate("external_directory", "C:\\Users\\test\\p\\ustcode\\src\\*", permissions).effect,
      ).toBe("allow")
