...
import { ConfigProviderPlugin } from "@enthusjast/ustcode-core/config/plugin/provider"
...
describe("ConfigProviderPlugin.Plugin", () => {
  it.effect("keeps configured model variant bodies unchanged", () =>
    Effect.gen(function* () {
      ...
      const providerID = ProviderV2.ID.ustcode
          ...
          Effect.succeed([
            new Config.Document({
              ...
              info: decode({
                providers: {
                  ustcode: {
                    api: { type: "aisdk", package: "@ai-sdk/openai", url: "https://ustcode.test/v1" },
                    models: {
                      "alpha-gpt-next": {
                        variants: [
                          {
