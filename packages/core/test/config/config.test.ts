...
import { ConfigProvider } from "@enthusjast/ustcode-core/config/provider"
...
import { ConfigMigrateV1 } from "@enthusjast/ustcode-core/v1/config/migrate"
...
import { Policy } from "@enthusjast/ustcode-core/policy"
...
describe("Config", () => {
  ...
  it.live("loads ustcode JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "ustcode.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "ustcode.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "ustcode.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "ustcode.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "ustcode.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "ustcode.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .ustcode configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".ustcode"), { recursive: true })
            await fs.mkdir(path.join(directory, ".ustcode"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "ustcode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "ustcode.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "ustcode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "ustcode.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "ustcode.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".ustcode", "ustcode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".ustcode", "ustcode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".ustcode")),
              AbsolutePath.make(path.join(directory, ".ustcode")),
            ])
