...
import { define } from "@enthusjast/ustcode-plugin/v2/effect"
...
import { LocationServiceMap } from "@enthusjast/ustcode-core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "ustcode.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "ustcode.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
