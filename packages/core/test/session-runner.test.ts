...
import * as OpenAIChat from "@enthusjast/ustcode-llm/protocols/openai-chat"
import { Database } from "@enthusjast/ustcode-core/database/database"
import { makeLocationNode } from "@enthusjast/ustcode-core/effect/app-node"
...
import { PermissionV2 } from "@enthusjast/ustcode-core/permission"
import { EventTable } from "@enthusjast/ustcode-core/event/sql"
import { Project } from "@enthusjast/ustcode-core/project"
import { ProjectTable } from "@enthusjast/ustcode-core/project/sql"
import { QuestionV2 } from "@enthusjast/ustcode-core/question"
import { AbsolutePath } from "@enthusjast/ustcode-core/schema"
import { SessionV2 } from "@enthusjast/ustcode-core/session"
import { Snapshot } from "@enthusjast/ustcode-core/snapshot"
import { ContextSnapshotDecodeError } from "@enthusjast/ustcode-core/session/error"
import { SessionEvent } from "@enthusjast/ustcode-core/session/event"
import { SessionInput } from "@enthusjast/ustcode-core/session/input"
import { SessionMessage } from "@enthusjast/ustcode-core/session/message"
import { Prompt } from "@enthusjast/ustcode-core/session/prompt"
...
import { SessionRunCoordinator } from "@enthusjast/ustcode-core/session/run-coordinator"
import { SessionRunner } from "@enthusjast/ustcode-core/session/runner"
import * as SessionRunnerLLM from "@enthusjast/ustcode-core/session/runner/llm"
import { SessionRunnerModel } from "@enthusjast/ustcode-core/session/runner/model"
import { ToolRegistry } from "@enthusjast/ustcode-core/tool/registry"
import { ApplicationTools } from "@enthusjast/ustcode-core/tool/application-tools"
import { AgentV2 } from "@enthusjast/ustcode-core/agent"
import { Config } from "@enthusjast/ustcode-core/config"
import { ConfigCompaction } from "@enthusjast/ustcode-core/config/compaction"
import { Tool } from "@enthusjast/ustcode-core/tool/tool"
import {
  SessionContextEpochTable,
  SessionInputTable,
  SessionMessageTable,
  SessionTable,
} from "@enthusjast/ustcode-core/session/sql"
import { SessionStore } from "@enthusjast/ustcode-core/session/store"
import { SystemContext } from "@enthusjast/ustcode-core/system-context"
import { SystemContextRegistry } from "@enthusjast/ustcode-core/system-context/registry"
import { SkillGuidance } from "@enthusjast/ustcode-core/skill/guidance"
import { ReferenceGuidance } from "@enthusjast/ustcode-core/reference/guidance"
...
describe("SessionRunnerLLM", () => {
  ...
  it.effect("runs different sessions concurrently", () =>
    Effect.gen(function* () {
      ...
      expect(requests).toHaveLength(2)
      // The core runner does not set providerOptions.promptCacheKey — that is a
      // per-provider, config-gated feature in the ustcode package (transform.ts).
      expect(requests.every((request) => request.providerOptions?.openai?.promptCacheKey === undefined)).toBe(true)
      yield* Deferred.succeed(streamGate, undefined)
      yield* Fiber.join(first)
      yield* Fiber.join(second)
      streamGate = undefined
  ...
  it.effect("resumes sessions without setting providerOptions.promptCacheKey", () =>
    Effect.gen(function* () {
      yield* setup
      ...
      // The core runner never populates promptCacheKey; the bounded-key logic
      // lives in the ustcode provider transform, gated on setCacheKey.
