...
import { Watcher } from "@enthusjast/ustcode-core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    USTCODE_EXPERIMENTAL_FILEWATCHER: "true",
    USTCODE_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@ustcode.test`.cwd(tmp.path).quiet()
