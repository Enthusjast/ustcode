...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@ustcode/acme")).toBe("@ustcode/acme")
    expect(Npm.sanitize("@ustcode/acme@1.0.0")).toBe("@ustcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/ustcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/ustcode/acme.git" : spec
