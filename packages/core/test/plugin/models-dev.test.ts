...
import { ModelsDevPlugin } from "@enthusjast/ustcode-core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.USTCODE_MODELS_PATH,
          disabled: Flag.USTCODE_DISABLE_MODELS_FETCH,
        }
        Flag.USTCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.USTCODE_MODELS_PATH = previous.path
          Flag.USTCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
