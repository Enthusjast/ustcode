...
export const USTCODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const USTCODE_MODE_KEY = "ustcode.mode"

export const UstcodeKeymapProvider = KeymapProvider
export const useUstcodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type UstcodeModeStack = ReturnType<typeof createUstcodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, UstcodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createUstcodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(USTCODE_MODE_KEY, USTCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(USTCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(USTCODE_MODE_KEY, stack.at(-1)?.mode ?? USTCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? USTCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(USTCODE_MODE_KEY, undefined)
...
export function useUstcodeModeStack() {
  return getUstcodeModeStack(useUstcodeKeymap())
}

export function getUstcodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Ustcode mode stack is not registered for this keymap")
  return value
}
...
export function registerUstcodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createUstcodeModeStack(keymap)
