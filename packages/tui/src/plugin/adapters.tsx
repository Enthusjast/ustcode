import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@enthusjast/ustcode-plugin/tui"
...
import type { useUstcodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useUstcodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getUstcodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getUstcodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
