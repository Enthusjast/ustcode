import type { TuiPluginApi } from "@enthusjast/ustcode-plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("ustcode.status"),
...
const TIPS: Tip[] = [
  ...
  "Create {highlight}ustcode.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/ustcode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.ustcode/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.ustcode/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.ustcode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.ustcode/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent UstCode from reading sensitive files",
  "Use {highlight}ustcode run{/highlight} for non-interactive scripting",
  "Use {highlight}ustcode --continue{/highlight} to resume the last session",
  "Use {highlight}ustcode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}ustcode serve{/highlight} for headless API access to UstCode",
  "Use {highlight}ustcode run --attach{/highlight} to connect to a running server",
  "Run {highlight}ustcode upgrade{/highlight} to update to the latest version",
  "Run {highlight}ustcode auth list{/highlight} to see all configured providers",
  "Run {highlight}ustcode agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.ustcode/themes/{/highlight} directory",
  ...
  "Run {highlight}ustcode debug config{/highlight} to troubleshoot configuration",
  ...
  "Use {highlight}/connect{/highlight} to configure USTC or another OpenAI-compatible provider",
