...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@enthusjast/ustcode-sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__ustcode_custom_provider__"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in ustcode.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={() =>
        ({
          ...
          "ustcode-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                UstCode Go is a $10 per month subscription that provides reliable access to popular open coding models
                with generous usage limits.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://ustcode.enthusjast.cc/go</span> and enable UstCode Go
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        try {
          ...
          toast.show({
            variant: "success",
            message: `Saved API key for ${props.providerID}`,
          })
          await sdk.client.instance.dispose()
          await sync.bootstrap()
          if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
            toast.show({
              variant: "info",
              message: `Saved credential for ${props.providerID}. Configure it in ustcode.json to use it.`,
            })
            dialog.clear()
            return
          }
