...
import { getUstcodeModeStack, USTCODE_BASE_MODE, UstcodeKeymapProvider, registerUstcodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerUstcodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when ustcode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getUstcodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getUstcodeModeStack(keymap).push("autocomplete")
