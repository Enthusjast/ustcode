...
describe("providerOptions", () => {
  ...
  test("keeps popular providers first and sorts the rest alphabetically", () => {
    // Only built-in providers (USTC / ustcode-go) are "popular"; everything
    ...
    expect(
      providerOptions([
        ...
        { id: "ustcode-go", name: "UstCode Go" },
      ]).map((option) => option.value),
    ).toEqual(["ustcode-go", "anthropic", "aws", "mistral", "openai", "custom-z", "__ustcode_custom_provider__"])
  })
